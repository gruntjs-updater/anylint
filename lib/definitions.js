...
function getListOfDefinitions(ast, type, definitions){
	...
	var filteredTokens = filter(tokens, definitions);
	return _.map(filteredTokens, extractor);
}
...
function filterOutExpressions(filePath, expressionType, names){
	...
	var filteredExpressions = getListOfDefinitions(ast,expressionType,names);
